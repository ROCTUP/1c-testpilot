# -*- coding: utf-8 -*-
"""Shared 1C operations for the Python API and MCP adapter.
...
# A direct Python session owns its pool and snapshots. Context-local routing also
# allows it to coexist with MCP and other Python sessions in the same process.
_execution = ContextVar('testpilot_execution', default=None)
_format_result = None


def _address_mode():
    return 'off' if _execution.get() is not None else _response.REF_MODE


def _connection_pool():
    context = _execution.get()
    return context.pool if context is not None else _pool


def _snapshot_storage():
    context = _execution.get()
    return context.snapshots if context is not None else _snapshot_store


def _journal_store():
    context = _execution.get()
    return context.logs if context is not None else _log_store


def _deliver(result, *, addrs=False):
    if _execution.get() is not None or _format_result is None:
        return result
    return _format_result(result, addrs=addrs)


class Runtime:
    """Resources for one Python client. Operations still use the common handlers."""
    def __init__(self):
        self.legacy = _connections.empty_state()
        self.pool = _connections.Pool(_state, limit=1, legacy=self.legacy)
        self.snapshots = _snapshots.Store()
        self.logs = _log_store
    ...
    def call(self, action, arguments):
        ...
        try:
            with _state.bind(self.legacy):
                return _dispatch_action(_ACTIONS[group], group, dict(action=action, **arguments))
        finally:
...
def _exe_version(path):
    """Platform version from a standard Windows, Linux or macOS installation path."""
...
def _find_1cv8(want_version=None):
    ...
    patterns = ([r'C:\Program Files\1cv8\*\bin\1cv8.exe',
                 ...
                 r'C:\Program Files (x86)\1cv8\*\bin\1cv8c.exe'] if os.name == 'nt' else
...
def tc_write_content_to_file(key: str, handle: str, filename: str = None,
    ...
    try:
        ...
        if filename and can_clear:
            # Cleanup must not replace an accepted save or its original failure.
            try:
...
def _synth_fdr(m):
    ...
    Истина -> e2 <ИндексФильтра> <ЧислоИмён> <имя1 d_> <имя2..N 9_>; Ложь -> e1 81 82 c1."""
    if m[:1] != b'\xe2':
        return {'result': 'false'}, []
    numbers = []; i = 1
    for _ in range(2):
        if i < len(m) and 0x81 <= m[i] <= 0x8a:
            numbers.append(m[i] - 0x81); i += 1
        else:
            size = tc1c.value_size(m, i)
            if not size or i + size > len(m) or m[i] not in (0x8b, 0x8d, 0x8f):
                raise ValueError('Invalid file-dialog integer')
            numbers.append(_synth_dec_int(m[i:i + size])); i += size
    a = {'result': 'true', 'filterIndex': str(numbers[0])}
    count = numbers[1]
    names = []
...
def tc_run_scenario(uilog: str = None, path: str = None) -> dict:
    ...
    def _attempt(target, action):
        try:
            ...
            details = exc.result() if isinstance(exc, tc1c.OperationError) else getattr(exc, 'details', {})
            if isinstance(exc, _batches.Failure):
    ...
    failure_code = steps[stopped_at].get('code') if stopped_at is not None else None
    return {'ok': all(s['ok'] for s in played) if played else False,
            ...
            **({'code': failure_code} if failure_code is not None else {}),
...
def _public_parameters(fn):
    for p in inspect.signature(fn).parameters.values():
        ...
        if fn.__name__ in ('tc_read_fields', 'tc_set_fields') and p.name in ('targets', 'entries'):
            p = p.replace(annotation=_batches.public_type(p.name, _address_mode()))
        if _address_mode() == 'id':
            if p.name == 'handle':
                continue
...
def _fmt_action(name, fn):
    ...
    if name in _ADDRESS_NOTES:
        doc += '\n' + _ADDRESS_NOTES[name][_address_mode()]
...
def _dispatch_connected_action_impl(acts, group, kw, connection_id=None):
    ...
    try:
        ...
        if isinstance(res, _screenshots.Screenshot):
            ...
            return _deliver(_screenshots.Screenshot(res.png, metadata))
        if _address_mode() == 'id' and _state.get('client') is not None:
...
def _dispatch_action(acts, group, kw):
    ...
    try:
        ...
        if name in ('connect', 'launch_client'):
            ...
            connection = _connection_pool().reserve('127.0.0.1' if launching else params['host'], params['port'],
        ...
        with _connection_pool().use(connection):
            ...
            try:
                ...
                failure = (exc.result() if isinstance(exc, tc1c.ConnectionFailure) else
                           {'ok': False, 'code': 'connection_failed', 'error': str(exc)})
                return _deliver(_profiles.redact({**failure, 'connection_id': connection.id}))
            finally:
